...
if __name__ == "__main__":
    ...
    parser.add_argument('--tag', help="split", default='1:1')
    parser.add_argument('--model_type', default='KTO')
    ...
    resultsDir = os.path.join('./', model_type+'_results', dataset,tag,"BETA{}".format(beta))
    os.makedirs(resultsDir, exist_ok=True)

    metricsPath = os.path.join(resultsDir,'{}_{}.csv'.format(eval_func,UpperRange))
    resultsPath = os.path.join(resultsDir,'means_{}.csv'.format(UpperRange))
    ...
    if recompute:
        for sample_id in tqdm(range(LowerRange,UpperRange,4)):
            ...
            for j in range (0,t): 
                    trg[j]
                    if "zephyr" in model_ref:
                        tmp = PickBest(output_text[j], trg[j])
                        text.append((tmp,output_text[j]))
                        texts.append((tmp,output_text[j]))
                    else:
