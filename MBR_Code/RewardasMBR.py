...
if __name__ == "__main__":
    ...
    parser.add_argument('--tag', help="dataset tag", default='1:1')
    parser.add_argument('--model_type', default='KTO')
    parser.add_argument('--model_ref',default='mistralai/Mistral-7B-Instruct-v0.1')
    parser.add_argument('--model_path',default='../model-based-mbr/KTO/cnndm/1:1/BETA0.1/checkpoint-3600')
    ...
    ref_model = AutoModelForCausalLM.from_pretrained(
                model_ref,torch_dtype=torch.bfloat16, device_map="auto"
            )
    policy_model = AutoModelForCausalLM.from_pretrained(
                model_path, torch_dtype=torch.bfloat16, device_map="auto"
            )
    tokenizer = AutoTokenizer.from_pretrained(
                model_ref, padding_size="left", use_fast=True
            )
    tokenizer.pad_token_id = tokenizer.eos_token_id
    ...
    for fileId in tqdm(range(LowerRange,UpperRange)):
        ...
        ranks_method1[sorted_indices_method1] = np.arange(len(Rewards))
    ...
    with open('../model-based-mbr/spearmansMBR.txt', 'a') as file:
