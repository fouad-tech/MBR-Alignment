...
if __name__ == "__main__":
    ...
    for fileId in tqdm(range(LowerRange,UpperRange)):
        ...
        bestScore.append(compute_evaluate(df.iloc[best_r_hyp]['text'],trg,None))
        meteor.append(compute_evaluate_meteor(df.iloc[best_r_hyp]['text'],trg,None))
        rouge.append(compute_evaluate_rouge(df.iloc[best_r_hyp]['text'],trg,None))
        rouge1.append(compute_evaluate_rouge1(df.iloc[best_r_hyp]['text'],trg,None))
    
    print('bertscore: ',sum(bestScore)/len(bestScore))
    print('meteor: ',sum(meteor)/len(meteor))
    print('rouge: ',sum(rouge)/len(rouge))
    print('rouge1: ',sum(rouge1)/len(rouge1))


    with open('../model-based-mbr/RewardAsUtility.txt', 'a') as file:
        # Append the data to the file
        file.write(f"metrics {args.model_type} for {dataset} bertscore: {sum(bestScore)/len(bestScore)} meteor: {sum(meteor)/len(meteor)}  rouge: {sum(rouge)/len(rouge)} rouge1: {sum(rouge1)/len(rouge1)}\n")
