...
def KTO(hypsRanked,src,nu,nd):
  samples = []
  for i in range(0,nd):
    samples.append((src,hypsRanked[i],'T'))
  for i in range(len(hypsRanked)-1,0,-2):
    samples.append((src,hypsRanked[i],'F'))
    if len(samples)==nd+nu:
        break
  
  return samples

def shuffle(df):
    shuffled_df = df #df.iloc[np.random.permutation(len(df))]
...
if __name__ == "__main__":
    ...
    for id in tqdm(range(LowerRange,UpperRange)):
        ...
        if dataset == 'squad_v2T':
            ...
            pairs = KTO(hypsRanked,src_input,nu,nd)
            for pair in pairs:
                rowsBW.append(pair)
    ...
    bwFilePath = '../model-based-mbr/PreferenceSetsSplitsKTO'
    columns = [
            "src",
            "sample",
            "type",
    ...
    bwFilePathTrain = os.path.join(bwFilePath,'{}_{}:{}_train.csv'.format(dataset,nd,nu))
    bwFilePathTest = os.path.join(bwFilePath,'{}_{}:{}_test.csv'.format(dataset,nd,nu))
