    ...
    if model_name == "None":
        ...
        if "polylm" in model_n:
            tokenizer = AutoTokenizer.from_pretrained(
                model_name, legacy=False, use_fast=False, padding_size="left"
            )
            if "polylm" in model_n:
                stop_tokens = [213]  # new line token \n
        else:
            tokenizer = AutoTokenizer.from_pretrained(
                model_name, padding_size="left", use_fast=True, trust_remote_code=True
            )
        ...
        if (
            ...
            or ("zephyr" in model_name)
            or ("mistral-7b-sft-beta" in model_name)
        ):
...
def load_dataset(dataset, ref=False):
    ...
    if not (subdir_name == "hf"):
        ...
        if dataset == "samsum":
            ...
            if not ref:
              lines = dataset['document']
            else:
              lines = dataset["summary"]
        elif dataset =='strategyqaT':
            df = pd.read_csv('../model-based-mbr/strategyqaData/strategyqaTrain.csv')
            df.fillna(
            "", inplace=True
            ...
            if not ref:
                ...
                lines = df.iloc[:]['target']

        elif dataset =='strategyqaV':
            df = pd.read_csv('../strategyqaData/strategyqaValidation.csv')
            df.fillna(
            "", inplace=True
            )  # TODO: This is needed to remove empty strings. In reality empty strings can be ignored. probably it's better to drop.
            if not ref:
            ...
            if not ref:
              lines = dataset['article']
            else:
              lines = dataset["highlights"]
...
def load_kwargs(dataset):
    ...
    STRAT_KWARGS = dict(max_new_tokens=75, no_repeat_ngram_size=4)
    ...
    elif dataset in ["strategyqaT","strategyqaV"]:
        model_kwargs = STRAT_KWARGS
    else:
...
def load_reward(target_matrix_dir, filename, sim, n_samples):
    """
    Load matrix from the target_matrix_dir.
    The matrix is saved as a text file.
    The matrix stored may be larger than n_samples. In this case, it is truncated.
    ...
    matrix = pd.read_csv(matrix_base + str(min_nsamples))
    result_dict = matrix.set_index('index')['reward'].to_dict()
    first_n_items = list(result_dict.items())[:n_samples]
    first_n_dict = dict(first_n_items)
...
def load_samples_from_file(
    files, epsilon, topk, topp, do_sample=True, diverse_k=1, divpen=0.0
):
